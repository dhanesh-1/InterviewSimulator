...
function isTechnicalRole(role) {
  ...
  // Normalize: lowercase, replace multiple spaces/tabs/newlines with a single space, trim
  ...
  // Allow UI/UX, product, web, graphic designers, etc., but reject general, fashion, interior, etc.
  if (/\bdesigner\b/.test(normalized)) {
    const allowedDesignContexts = ['ui', 'ux', 'web', 'product', 'interaction', 'digital', 'interface', 'graphic', 'graphics', 'software', 'app', 'application', 'game'];
    const hasAllowedContext = allowedDesignContexts.some(context => {
      const regex = new RegExp(`\\b${context}\\b`);
  ...
  // Allow engineering, technical, product, project, software, IT managers, but reject office, hotel, store, etc.
  if (/\bmanager\b/.test(normalized)) {
    const allowedManagerContexts = ['software', 'engineering', 'technical', 'it', 'tech', 'devops', 'qa', 'test', 'systems', 'database', 'network', 'product', 'cloud', 'project', 'scrum', 'agile'];
    const hasAllowedContext = allowedManagerContexts.some(context => {
      const regex = new RegExp(`\\b${context}\\b`);
      return regex.test(normalized);
    });
    if (!hasAllowedContext) {
  ...
  // 5. Additional check for general IT/Software terms if no specific role keyword was matched
  const techIndicators = ['software', 'frontend', 'backend', 'fullstack', 'full stack', 'cloud', 'cybersecurity', 'database', 'network', 'information technology', 'it'];
...
module.exports = { isTechnicalRole };
